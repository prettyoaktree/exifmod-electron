...
- **New** — Pending edits. When metadata **definitively matches** catalog presets (same identity rules as **Current**), the **New** column can **auto-select** those presets for Camera/Lens/Film/Author so the row reflects an already-aligned catalog choice; Film and Author matching are always evaluated independently of fixed vs interchangeable camera. When you **Save** in **New Preset** or **Edit Preset**, that preset is applied to the current staged selection for its category, then the catalog refreshes (auto-matching from file metadata still applies afterward where rules allow). Preset pickers (or “None” / **Do Not Modify**) are **searchable**: type to filter the list, or open it with the chevron; **Tab** and **Arrow** keys still move between metadata fields when the list is closed.
