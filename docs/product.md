...
2. **Select one or more files** in the list (or move **keyboard focus** only: with **no** selection, metadata and preview follow the **focused** row; with a **selection**, metadata and preview follow the **highlighted** rows while focus shows where **Space** will toggle next). The main window is split edge-to-edge into **files** (list + preview) and **metadata**; narrow dividers between sections can be dragged to resize. The UI shows a **preview** (when one image is staged) and a **metadata** area.
