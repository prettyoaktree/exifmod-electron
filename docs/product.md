...
- **New** — Pending edits. When metadata **definitively matches** catalog presets (same identity rules as **Current**), the **New** column can **auto-select** those presets for Camera/Lens/Film/Author so the row reflects an already-aligned catalog choice; Film and Author matching are always evaluated independently of fixed vs interchangeable camera. After you **create or save** a preset, the catalog refreshes and matching runs again with the same rules, so a new preset that fits the file can appear selected without manual picking. Preset pickers (or “None” / **Do Not Modify**) are **searchable**: type to filter the list, or open it with the chevron; **Tab** and **Arrow** keys still move between metadata fields when the list is closed.
