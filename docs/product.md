...
- **Description** and **Keywords** — **Description** maps to EXIF **ImageDescription** (UTF‑8 byte limit enforced on write). **Keywords** are comma- or line-separated tokens merged with preset **Keywords** on write (film stock markers, deduplication, and total size limits apply). For both, an empty **New** field means **do not change** what is on the file (no manual write for that field from this row).
- **Copy** (Description/Keywords rows) — Copies the row’s **Current Value** into **New Value**. Enabled only when the current value is a single usable value for the selection (disabled when Current shows **Multiple**). For Keywords, film-identifying tokens already on file (`film`, `… Film Stock`) are preserved by this path.
