...
| `Keywords` | **`mergeKeywordsDeduped`**(preset merged `Keywords`, parsed **Keywords** field when non-empty). Case-insensitive dedupe; preset tokens first, then tokens from the field (comma / line separated). Empty **Keywords** field means no extra tokens from the UI (same idea as shutter/aperture empty = no write); **Remove** forces full clear | Keywords |

The metadata pane uses **two** four-column tables (Attribute \| Current \| New \| **Remove**). The **Description and keywords** subsection title groups the second table and the **AI** control. **Remove** checkboxes are tri-state when multiple files are staged and per-file clear flags disagree: the native checkbox **indeterminate** state is used; the next click sets a single boolean for **all** staged files.

The **Description and keywords** header includes an **AI** control when at least one staged file has remaining ImageDescription room. The renderer passes `remainingUtf8BytesForAiDescription` an **effective** description string: pending **New** text if non-empty, otherwise the on-file description from metadata (so “Do Not Modify” still reserves bytes correctly). **Single-file** selection runs AI immediately. **Multi-file** selection opens a confirmation dialog, then invokes Ollama **sequentially** per file (each call uses a **640px** max-edge JPEG preview, same decode path as the thumbnail). Results **append** to each file’s description (respecting **`IMAGEDESCRIPTION_MAX_UTF8_BYTES`** and separator bytes via `mergeImageDescriptionAppend`) and **merge** into **Keywords** (`mergeKeywordsDeduped` + **`fitKeywordsForExif`**). The model receives the **remaining UTF‑8 byte budget** per file (`remainingUtf8BytesForAiDescription`). **Errors on one file do not abort the batch**; after the run, a dialog can list failures and offer **retry failed files**. The HTTP client only allows **loopback** hosts (`127.0.0.1`, `localhost`, `::1`). Default base URL `http://127.0.0.1:11434`, default model **`gemma4`** (overridable via `EXIFMOD_OLLAMA_HOST` / `EXIFMOD_OLLAMA_MODEL`). On launch, the main process runs a **text-only** `/api/chat` warmup to the same model; if warmup fails and the **`ollama`** CLI is not on PATH, the AI control stays off for the session. If warmup fails but the CLI exists, the renderer shows a **collapsible drawer** next to the AI button to start **`ollama serve`** (no modal); **Not now** collapses the drawer, which the user can expand again. On app quit, EXIFmod stops the **`ollama serve`** process **only** if it spawned it in that session after opt-in; it does not stop Ollama that was already running at launch.

Empty shutter/aperture fields mean **do not write** those tags (unless the **Remove** checkbox is checked for that row, which writes empty assignments to clear them). Empty **Description** / **Keywords** fields likewise mean **no manual write** from those controls (unless **Remove** is checked for that row).
