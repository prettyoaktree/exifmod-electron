...
- Preview + Ollama: `src/main/previewImage.ts` (640px max edge JPEG for the file list; **384px** max edge for Ollama vision input only), `src/main/ollamaDescribe.ts` (loopback-only `/api/chat` with `think: false` and bounded `options`; high-level-scene prompt; optional `maxDescriptionUtf8Bytes` from remaining ImageDescription space), `src/main/ollamaListVision.ts` (lists installed models with `vision` via `/api/tags` + `/api/show`), `src/main/ollamaPrefs.ts` (saved model in `userData/ollama-prefs.json`; `**EXIFMOD_OLLAMA_MODEL**` wins over the panel), `src/main/ollamaLifecycle.ts` (cached `**ollama:startupFlow**` vs uncached `**ollama:checkAvailability**` after describe transport failures)
